...
import           Crypto.Saltine.Internal.AEAD.ChaCha20Poly1305      (c_aead, c_aead_open, c_aead_detached, c_aead_open_detached)
...
instance IsEncoding Key where
  decode v = if S.length v == Bytes.aead_chacha20poly1305_keybytes
           then Just (Key v)
           else Nothing
  {-# INLINE decode #-}
  encode (Key v) = v
...
instance IsEncoding Nonce where
  decode v = if S.length v == Bytes.aead_chacha20poly1305_npubbytes
           then Just (Nonce v)
           else Nothing
  {-# INLINE decode #-}
  encode (Nonce v) = v
...
instance IsNonce Nonce where
  zero            = Nonce (S.replicate Bytes.aead_chacha20poly1305_npubbytes 0)
